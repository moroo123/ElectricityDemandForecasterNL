...
def objective(trial: optuna.trial.Trial, config: dict, df_trainval: pd.DataFrame, df_weather: pd.DataFrame | None):
    ...
    hpo_train_params = config['train']
    hpo_model_params = config['model']

    # Suggest values for hyperparameters
    lr = trial.suggest_float(
        "learning_rate", **hpo_train_params['learning_rate'])
    weight_decay = trial.suggest_float(
        "weight_decay", **hpo_train_params['weight_decay'])
    batch_size = trial.suggest_categorical(
        "batch_size", **hpo_train_params['batch_size'])

    model_kwargs = {k: v for k, v in hpo_model_params.items() if k not in [
        'name']}

    model_kwargs['lstm_hidden_size'] = trial.suggest_categorical(
        "lstm_hidden_size", **hpo_model_params['lstm_hidden_size'])
    model_kwargs['lstm_num_layers'] = trial.suggest_categorical(
        "lstm_num_layers", **hpo_model_params['lstm_num_layers'])
    model_kwargs['lstm_dropout'] = trial.suggest_float(
        'lstm_dropout', **hpo_model_params['lstm_dropout'])
    model_kwargs['cnn_conv_channels'] = trial.suggest_categorical(
        'cnn_conv_channels', **hpo_model_params['cnn_conv_channels'])
    model_kwargs['cnn_num_conv_layers'] = trial.suggest_categorical(
        'cnn_num_conv_layers', **hpo_model_params['cnn_num_conv_layers'])
    model_kwargs['cnn_conv_dropout'] = trial.suggest_float(
        'cnn_conv_dropout', **hpo_model_params['cnn_conv_dropout'])
    model_kwargs['cnn_kernel_size'] = trial.suggest_categorical(
        'cnn_kernel_size', **hpo_model_params['cnn_kernel_size'])
...
def main(db_path: str, table_name: str, db_path_weather: str | None = None, table_name_weather: str | None = None, config_path: str = 'configs', runs_root: str = 'runs', run_optimization: bool = True, fraction: int = 1):
    ...
    if run_optimization:
        ...
        study.optimize(
            ...
            n_trials=config['train']['hpo_trials']
        )
